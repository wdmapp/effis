from .util import InputList, Input
    ...
    def __init__(self, value, outpath=None, rename=None):
        if outpath is None:
            CompositionLogger.RaiseError(
                ValueError,
                "Need to give an outpath for Backup SendData"
            )
...
class Backup:
    ...
    def SetSourceEndpoint(self, filename=None):
        if filename is None:
            filename = os.path.join(
                os.environ["HOME"], ".effis-source-endpoint"
            )
        if not os.path.exists(filename):
            print(
                "The Globus endpoint ID for the current source host has not"
                "been set in EFFIS."
                "\n"
                "(UUIDs can be searched at https://app.globus.org/collections)"
                "\n"
    ...
    def __setitem__(self, key, item):
        ...
        if type(item) is Destination:
            ...
            CompositionLogger.RaiseError(
                ValueError,
                "Bad value for Backup: key={0}, value={1}".format(key, item)
            )
    ...
    def __init__(self, value=None):
        ...
        if value is None:
            ...
            CompositionLogger.RaiseError(
                ValueError,
                "Can only initialize Backup with None"
            )
