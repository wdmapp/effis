    ...
    def ExistenceChecks(self):
        ...
        if not os.path.exists(self.ConfigFile):
            CompositionLogger.Warning(
                "Can't configure campaign management: "
                "{0} does not exist".format(self.ConfigFile)
            )
            return False
        ...
        if self.ManagerCommand is None:
            ...
            if self.ManagerCommand is None:
                CompositionLogger.Warning(
                    "Can't configure campaign management: "
                    "{0} is not in $PATH".format(default)
                )
            return False
        ...
        if 'Campaign' not in config:
            CompositionLogger.Warning(
                "Key 'Campaign' not found in {0}. "
                "Skipping campaign management.".format(self.ConfigFile)
            )
            return False
        elif 'campaignstorepath' not in config['Campaign']:
            CompositionLogger.Warning(
                "Key 'campaignstorepath' not found under 'Campaign' in {0}. "
                "Skipping campaign management.".format(self.ConfigFile)
            )
            return False
        ...
        if isinstance(name, self.__class__):
            ...
            if (ConfigFile is not None) and (not os.path.exists(ConfigFile)):
                CompositionLogger.RaiseError(
                    ValueError,
                    "ConfigFile={0} does not exist.".format(ConfigFile)
                )
            ...
            if (ManagerCommand is not None):
                if (
                    os.path.exists(ManagerCommand)
                    or
                    (shutil.which(ManagerCommand) is not None)
                ):
                    ...
                    CompositionLogger.RaiseError(
                        ValueError,
                        (
                            "ManagerCommand={0} is invalid: must be in $PATH "
                            "or an existing file path".format(ManagerCommand)
                        )
                    )
            ...
            if (name is not None) and not isinstance(name, str):
                CompositionLogger.RaiseError(
                    ValueError,
                    "Can olny set Campaign name with a string"
                )
