...
def ValidateIntOptions(options, Application, label="Application"):
    ...
    for name in options:
        if (
            (name in Application.__dir__())
            and
            (getattr(Application, name) is not None)
        ):
            if not isinstance(getattr(Application, name), (str, int)):
                CompositionLogger.RaiseError(
                    AttributeError,
                    (
                        "{0} {1} setting must be an integer "
                        "(or string of one)".format(name, label)
                    )
                )
...
class UseRunner(object):
    """
    The idea of UseRunner inheritance is an abstraction on
    Workflow and Application setup. The two are separate,
    but use a common backend infrastructure for attribute configuration.
    Each takes a Parallel Runner child class object (or None)
    to configure appropriately for the system/situation.
    """
    ...
    def DetectRunnerInfo(cls, useprint=True):
        ...
        if Detected.System is False:
            ...
            try:
                machine = socket.getaddrinfo(
                    socket.gethostname(), 0,
                    flags=socket.AI_CANONNAME)[0][3].lower(
                    )
            ...
            if useprint and (Detected.System is not False):
                msg = "DetectRunnerInfo: Found {0}".format(
                    Detected.System.__class__.__name__
                )
    ...
    def UnknownError(self, key):
        ...
        CompositionLogger.RaiseError(
            AttributeError,
            "{0} is not an attribute for {1} using Runner={2}".format(
                key, self.__class__.__name__, rname
            )
        )
    ...
    def __setattr__(self, name, value):
        ...
        if name not in self.__dir__():
            ...
            CompositionLogger.RaiseError(
                AttributeError,
                (
                    "For {1}, not allowed to explicitly set private attribute "
                    "{0}".format(name, self.__class__.__name__)
                )
            )
    ...
    def __init__(self, **kwargs):
        """
        Set up with the proper kind of Runner
        and only allow setting known attributes
        """

        if "Runner" not in kwargs:
            CompositionLogger.Warning(
                "Runner was not set with {0} ({1}). Detecting what to use..."
                "".format(self.__class__.__name__, self.kwargsmsg(kwargs))
            )
            super().__setattr__(
                'Runner', self.DetectRunnerInfo(useprint=False)
            )
            if self.Runner is None:
                self._RunnerError_[0](self._RunnerError_[1])
            else:
                CompositionLogger.Info(
                    "Using detected runner {0}".format(self.Runner.cmd)
                )
        else:
        ...
        # including applying any special classes that are designed to handle
        # how arguments are given as easier inputs
        ...
        for key in self.__dir__():
            ...
            if (
                ...
                and
                not (key.startswith("_") and key.endswith("_"))
            ):
        ...
        if isinstance(other, type(self)):
            ...
            for i in range(len(other)):
                if not isinstance(other[i], type(self)):
                    CompositionLogger.RaiseError(
                        ValueError,
                        "Elements to add must be of type {0}".format(
                            self.__class__.__name___
                        )
                    )
            left = [self]
            right = other
            ...
            CompositionLogger.RaiseError(
                ValueError,
                (
                    "Can only add another {0} object (or lists of them) to {0}"
            ...
            for item in getattr(self, kind):
                ...
                if item.rename is not None:
                    ...
                    outpath = os.path.join(
                        outpath, os.path.basename(item.inpath)
                    )
...
class ParallelRunner:
    """
    All Batch Queues and MPI (or other multiprocess) Launchers
    ...
    def Validate(self, Options):
        ...
        if shutil.which(self.cmd) is None:
            CompositionLogger.RaiseError(
                ValueError, "{0} was not found".format(self.cmd)
            )
    ...
    def GetCall(self, Options, Extra=Arguments([])):
        ...
        for option in self.options:
            if getattr(Options, option) is not None:
                RunnerArgs += [
                    self.options[option], str(getattr(Options, option))
                ]
            ...
            for name in ('RanksPerNode', 'GPUsPerRank'):
                if getattr(Application, name) is not None:
                    CompositionLogger.RaiseError(
                        AttributeError,
                        (
                            "Setting {0} without setting Ranks is ambiguous"
                            "".format(name)
                        )
                    )
            CompositionLogger.Warning(
                "Ranks was not set for Application name={0}. "
                "Setting it to 1 (with {1})".format(Application.Name, cls.cmd)
            )
            Application.Ranks = 1
    ...
    def ValidateOptions(cls, Workflow):
        ...
        if Workflow.Output is None:
            Workflow.Output = os.path.join(
                Workflow.Directory, "{0}-%J.out".format(Workflow.Jobname)
            )
    ...
    def ValidateOptions(cls, Workflow):
        for name in ('Charge', 'Walltime', 'Nodes'):
            if getattr(Workflow, name) is None:
                CompositionLogger.RaiseError(
                    AttributeError,
                    "{0}: Summit workflow must set {1}".format(
                        Workflow.Name, name
                    )
                )
...
class jsrun(ParallelRunner):
    ...
    def ValidateOptions(cls, Application):
        ValidateIntOptions(
            ("nrs", "RanksPerRs", "CoresPerRs", "GPUsPerRs", "RsPerNode"),
            Application
        )
...
class slurm(ParallelRunner):
    ...
    def Monitor(cls, jobid, Name):
        ...
        '''
        sacct \
                --jobs 38357777 --allocations --parsable \
        ...
        if len(ResultList) == 0:
            # This should only happend with dependencies,
            # which sacct doesn't record until the dependent is done
            state = "DEPENDENT"
        elif len(ResultList) > 1:
            CompositionLogger.RaiseError(
                RuntimeError,
                (
                    "jobid {0} had more than one scct result. "
                    "This shouldn't happen.".format(jobid)
                )
            )
        elif len(ResultList) == 1:
        ...
        for badstate in BadStates:
            if state.find(badstate) != -1:
                CompositionLogger.RaiseError(
                    RuntimeError,
                    (
                        "jobid state {0} was '{2}', and Worklow Name={1} "
                        "depends on it".format(jobid, Name, state)
                    )
                )
        ...
        if state.find("COMPLETED") != -1:
            ...
            CompositionLogger.RaiseError(
                RuntimeError,
                "Unknown job state occured {0}: state={1}".format(jobid, state)
            )
    ...
    def ValidateOptions(cls, Workflow):
        for name in ('Charge', 'Walltime', 'Nodes', 'Constraint'):
            if getattr(Workflow, name) is None:
                CompositionLogger.RaiseError(
                    AttributeError,
                    "{0}: Perlmutter workflow must set {1}".format(
                        Workflow.Name, name
                    )
                )
        super().ValidateOptions(Workflow)
    ...
    def ValidateOptions(cls, Workflow):
        for name in ('Charge', 'Walltime', 'Nodes'):
            if getattr(Workflow, name) is None:
                CompositionLogger.RaiseError(
                    AttributeError,
                    "{0}: Frontier workflow must set {1}".format(
                        Workflow.Name, name
                    )
                )
        super().ValidateOptions(Workflow)
    ...
    def ValidateOptions(cls, Workflow):
        for name in ('Charge', 'Walltime', 'Nodes'):
            if getattr(Workflow, name) is None:
                CompositionLogger.RaiseError(
                    AttributeError,
                    "{0}: Andes workflow must set {1}".format(
                        Workflow.Name, name
                    )
                )
        super().ValidateOptions(Workflow)
...
class srun(ParallelRunner):
    ...
    def ValidateOptions(cls, Application):
        if (
            (Application.GPUsPerRank is not None)
            and
            (Application.RanksPerGPU is not None)
        ):
            CompositionLogger.RaiseError(
                AttributeError,
                "{0}: Can only set one of GPUsPerRank and RanksPerGPU".format(
                    Application.Name
                )
            )
        ValidateIntOptions(cls.options, Application)
...
class srun2jsrun(srun):
    ...
    def CoresNodesAdd(RunnerArgs, Options, RanksPerRs=1, nrs=1):
        ...
        if Options.CoresPerRank is not None:
            ...
            RunnerArgs += [
                jsrun.options['bind'],
                "packed:{0}".format(Options.CoresPerRank)
            ]
        else:
            CompositionLogger.Warning(
                "Cannot determine CoresPerRs for Application Name={0}".format(
                    Options.Name
                )
            )
        ...
        if Options.RanksPerNode is not None:
            if Options.RanksPerNode % RanksPerRs != 0:
                CompositionLogger.RaiseError(
                    AttributeError,
                    "RanksPerNode={0} not divisible by RanksPerRs={1}".format(
                        Options.RanksPerNode, RanksPerRs
                    )
                )
            ...
            if nrs % Options.Nodes != 0:
                CompositionLogger.RaiseError(
                    AttributeError,
                    "nrs={0} not divisible by Nodes={1}".format(
                        nrs, Options.Nodes
                    )
                )
            ...
            CompositionLogger.Warning(
                "Cannot determine RsPerNode for Application Name={0}".format(
                    Options.Name
                )
            )
    ...
    def GetCall(self, Options, Extra=Arguments([])):
        ...
        if Options.RanksPerGPU is not None:
            if Options.Ranks % Options.RanksPerGPU != 0:
                CompositionLogger.RaiseError(
                    AttributeError,
                    "Ranks={0} not divisible by RanksPerGPU={1}".format(
                        Options.Ranks, Options.RanksPerGpu
                    )
                )
            ...
            self.CallMap(
                ...
                nrs=nrs,
                GPUsPerRs=1,
                RanksPerRs=Options.RanksPerGPU
            )
