...
class AuthorizedTranserClient:
    ...
    # e.g. NERSC DTN requires adding a GCS "data access" to the TransferScopes
    # (which effectively anything that does anything needs)
    ConsentRequired = {
        'required_scopes': [
            TransferScopes.all,
            ...
            (
                ...
                "5fac2e64-c734-4e6b-90ea-ff12ddbf9653/notification_notify"
            ),
        ]
    }

    # OLCF DTN uses such authorizations,
    # which can be caught in exception handling, like the consents
    ...
    def GetNewTokenResponse(self):
        ...
        token_response = self.auth_client.oauth2_exchange_code_for_tokens(
            auth_code
        )

        # This saves credentials with access to the (Native) App.
        # New ones are needed if new consents are needed, but the Globus
        # registered app appends new consents to persist (unless revoked)
    ...
    def ServerFromFile(self, server):
        ...
        try:
            self.ByResourceServer[server] = (
                self.auth_client.oauth2_refresh_token(
                    tokens["refresh_token"]
                ).by_resource_server[server]
            )
        except globus_sdk.services.auth.errors.AuthAPIError:
            print(
                "The saved credentials that were loaded didn't work. "
                "Getting new ones..."
            )
    ...
    # See https://globus-sdk-python.readthedocs.io/en/stable/tokenstorage.html
    # (if ever confidential app client needed)
    ...
    # There are other kinds of clients too (like Group, Auth)
    # and can take tokens from different resource servers
    def GetTransferClient(self):
        self.TransferClient = globus_sdk.TransferClient(
            authorizer=self.RefreshTokenAuthorizer("transfer.api.globus.org")
        )

    def AutoActivate(self, idhash, label):
        r = self.TransferClient.endpoint_autoactivate(
            idhash,
            if_expires_in=3600
        )
        url = "https://app.globus.org/file-manager?origin_id={0}".format(
            idhash
        )
        first = True
        # print(idhash, r)
        while (
            (r['code'] == "AutoActivationFailed")
            or
            ...
            if first:
                print(
                    ...
                    "please open the following URL in a browser to activate "
                    "the endpoint:".format(label) + "\n" + url
                )
    ...
    def CheckConsents(self):
        ...
        for endpoint in (
            [self.SourceEndpoint] + list(self.DestinationEndpoints)
        ):
                ...
                if (
                    ...
                    if err.info.authorization_parameters:
                        for key in self.AuthorizationParameters:
                            entry = (
                                err.info.authorization_parameters.__dict__[key]
                            )
                            if (type(entry) is list) and (len(entry) > 0):
                                print(
                                    "{0} -- Extra authorizations required: "
                                    "{1}-->{2}".format(endpoint, key, entry)
                                )
        ...
        if redo:
            self.auth_client.oauth2_start_flow(
                refresh_tokens=True,
                requested_scopes=self.ConsentRequired['required_scopes']
            )
            self.GetNewTokenResponse()
    ...
    def __init__(
        ...
        *DestinationEndpoints,
        ...
        data_access_scope = GCSCollectionScopeBuilder(nerscdtn).make_mutable(
        ...
        auth_client.oauth2_start_flow(
            refresh_tokens=True, requested_scopes=transfer_scope
        )
        """
        ...
        self.AuthClient = globus_sdk.AuthClient(
            authorizer=self.RefreshTokenAuthorizer("auth.globus.org")
        )
        self.ActionsClient = globus_sdk.AuthClient(
            authorizer=self.RefreshTokenAuthorizer("actions.globus.org")
        )
