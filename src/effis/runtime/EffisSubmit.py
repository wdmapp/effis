...
def main():
    ...
    if args.name is not None:
        filename = os.path.join(
            args.directory, "{0}.workflow.pickle".format(args.name)
        )
        ...
        for name in names:
            if (
                name.endswith("workflow.pickle")
                and
                (not name.endswith("sub.workflow.pickle"))
            ):
                filename = name
                count += 1
        ...
        if count == 1:
            ...
            raise NameError(
                "More than one workflow description found. Set one with --name"
            )
        elif count == 0:
    ...
    if not os.path.exists(filename):
        raise ValueError(
            "No workflow description found in {0}".format(args.directory)
        )
